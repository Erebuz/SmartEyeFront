      ...
      <v-col cols='4'>
        ...
        <v-row class='mt-2'>
          <v-col cols='6' class=' d-flex align-center'>
            <div>Максимум FPS: <b>{{video.fps.max}}</b></div>
          </v-col>
          ...
          <v-col cols='6'  class='d-flex align-center'>
            <div>Пропускает кадров: <b>{{store.getters.getNN.skip}}</b></div>
          </v-col>

          <v-col cols='6'  class=' d-flex align-center'>
              <v-text-field v-model.number='target_fps' type='number' :min='5' :max='1000' label='Целевой FPS' hide-details density='compact' variant='outlined' />
          </v-col>
          ...
          <v-col cols='6'  class='d-flex align-center'>
              <v-text-field v-model.number='skip' type='number' :min='0' :max='10' l label='Пропуск кадров' hide-details density='compact' variant='outlined'  />
          </v-col>
        ...
        <v-row>
          <v-col cols='6'  class='d-flex align-center'>
            <v-switch v-model='enable_nn' label='Нейросеть включена' hide-details />
          </v-col>
        </v-row>
        ...
        <v-row>
          <div v-for='class_name in Object.keys(store.getters.getNN.classes)' :key='class_name' style='width: 33%'>
            <v-checkbox v-model='store.getters.getNN.classes[class_name]' :label='class_name[0].toUpperCase() + class_name.slice(1)' density='compact' @change='set_classes' />
          </div>
        </v-row>
